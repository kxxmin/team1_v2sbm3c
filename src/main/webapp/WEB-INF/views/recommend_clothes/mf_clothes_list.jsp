...
<title>http://localhost:9091/recommend_clothes/mf_clothes.do</title> 
...
function send() {
  ...
  $.ajax({
    url: 'http://localhost:8000/recommend_clothes/mf_clothes', // form action
    ...
    success: function(rdata) { // 응답이 온경우
      ...
      for (var i=0; i< rdata.length; i++) {  // 0 ~ 2
    	  tags += "<DIV style='margin: 0px auto; width: 19%; float:left;'>";
        let row = rdata[i]; // 배열에서 하나의 객체 추출 ★
        tags += "clothes_id: " + row.clothes_id + "<br>";
        tags += "clothes_title:" + row.clothes_title + '<br>';
        tags += "clothes_rating:" + row.clothes_rating + '</li>';
        tags += "</DIV>";
      }
      tags += "</DIV>";
  ...
  $('#panel').html("<img src='/recommend_clothes/images/ani04.gif' style='width: 10%;'>"); // static
