...
public class EventProc implements EventProcInter {
  ...
  public ArrayList<EventVO> list_by_cateno(int cateno) {
    ArrayList<EventVO> list = this.eventDAO.list_by_cateno(cateno);
    
    for(int i = 0; i < list.size(); i++) {
      EventVO eventVO = list.get(i);
      
      String title = eventVO.getEvtitle();
      String content = eventVO.getEvdesc();
      
      title = Tool.convertChar(title);
      content = Tool.convertChar(content);
      
      eventVO.setEvtitle(title);
      eventVO.setEvdesc(content);
    }
    return list;
  }
  ...
  public EventVO read(int evtno) {
    EventVO eventVO = this.eventDAO.read(evtno);
  ...
  public ArrayList<EventVO> list_by_cateno_search(HashMap<String, Object> hashmap) {
    ArrayList<EventVO> list = this.eventDAO.list_by_cateno_search(hashmap);
  ...
  public ArrayList<CateEventVO> list_all() {
    ArrayList<CateEventVO> list = this.eventDAO.list_all();
    return list;
  }
  ...
  public ArrayList<EventVO> list_by_cateno_search_paging(HashMap<String, Object> map) {
    ...
    */
    ...
    ArrayList<EventVO> list = this.eventDAO.list_by_cateno_search_paging(map);
...
//      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
      ...
      if (now_grp >= 2){ // 현재 그룹번호가 2이상이면 페이지수가 11페이 이상임으로 이전 그룹으로 갈수 있는 링크 생성 
        str.append("<span class='span_box_1'><A href='"+Contents.LIST_FILE+"?&evword="+evword+"&now_page="+_now_page+"&cateno="+cateno+"'>이전</A></span>"); 
      } 
      ...
      for(int i=start_page; i<=end_page; i++){ 
        ...
        if (now_page == i){ // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
          ...
          str.append("<span class='span_box_1'><A href='"+Contents.LIST_FILE+"?evword="+evword+"&now_page="+i+"&cateno="+cateno+"'>"+i+"</A></span>");   
        } 
      } 
      ...
      if (now_grp < total_grp){ 
        str.append("<span class='span_box_1'><A href='"+Contents.LIST_FILE+"?&evword="+evword+"&now_page="+_now_page+"&cateno="+cateno+"'>다음</A></span>"); 
      } 
      str.append("</DIV>"); 
    ...
    @Override
    public int delete(int evtno) {
      int cnt = this.eventDAO.delete(evtno);
      return cnt;
    }
}
