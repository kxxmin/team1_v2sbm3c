...
public class ContentsCont {
  ...
  // 등록 폼
  // http://localhost:9093/contents/create.do?cateno=1
...
//  /**
//   * 모든 레코드 목록, http://localhost:9093/contents/list_by_cateno_search_paging.do?cateno=1
//   * @return
//   */
...
//  }
  ...
  // MAP 등록/수정/삭제 폼
  // http://localhost:9093/contents/map.do?cateno=15&contentsno=6
  ...
  // Youtube 등록/수정/삭제 폼
  // http://localhost:9093/contents/youtube.do?cateno=15&contentsno=6
...
//  /**
//   * 검색 레코드 목록, http://localhost:9093/contents/list_by_cateno_search_paging.do?cateno=1
//   * @return
//   */
//  @RequestMapping(value="/contents/list_by_cateno_search_paging.do", method=RequestMethod.GET)
...
//  }
  ...
  /**
   * cate + contents INNER JOIN 목록, http://localhost:9093/contents/list_all.do
   * @return
   */
...
public ModelAndView list_by_cateno_search_paging_cookie(
 ...
 return mav;
  ...
  /**
   * Grid 형태의 화면 구성 http://localhost:9093/contents/list_by_cateno_grid.do
   * 
   * @return
   */
  ...
  /**
   * 수정 폼
   * http://localhost:9093/contents/update_text.do?contentsno=1
   * 
   * @return
   */
  ...
  /**
   * 파일 수정 폼
   * http://localhost:9093/contents/update_file.do?contentsno=1
   * 
   * @return
   */
  ...
  /**
   * 파일 수정 처리 http://localhost:9093/contents/update_file.do
   * 
   * @return
   */
  ...
  /**
   * 삭제 처리 http://localhost:9093/contents/delete.do
   * 
   * @return
   */
...
/**
 * 추천 목록, http://localhost:9091/contents/mf_movie_member.do
 * @return
 */
@RequestMapping(value="/contents/mf_clothes_member.do", method=RequestMethod.GET)
public ModelAndView mf_clothes_member(HttpSession session) {
  ModelAndView mav = new ModelAndView();
  
  // Spring boot -> Django로 요청을 보냄 -> JSON 문자열 수신
  int memberno = (int)session.getAttribute("memberno");
  //int userId = 1; //이상태에서 실행 1번만 나오게 하려면
  int userId = memberno;
  
  String source ="";
  try {
    source = mf_recommend("http://127.0.0.1:8000/recommend_clothes/mf_clothes?userId=" + userId);
  ...
  // -> source[{"movie_id": 246, "movie_title": "Hoop Dreams (1994)", "movie_rating": 5}, {"movie_id": 318, "movie_title": "Shawshank Redemption, The (1994)", "movie_rating": 5}, {"movie_id": 720, "movie_title": "Wallace & Gromit: The Best of Aardman Animation (1996)", "movie_rating": 5}, {"movie_id": 741, "movie_title": "Ghost in the Shell (Kôkaku kidôtai) (1995)", "movie_rating": 5}, {"movie_id": 750, "movie_title": "Dr. Strangelove or: How I Learned to Stop Worrying and Love the Bomb (1964)", "movie_rating": 5}]
  
  // 옷 번호를 추출
  HashMap<String, Object> hashMap = new HashMap<String, Object>();
  
  JSONArray json = new JSONArray(source);
  ArrayList<String> contentsno_list = new ArrayList<String>(); 
  
  for (int index =0; index < json.length(); index ++) {
    JSONObject obj = (JSONObject)json.opt(index);
    String clothes_id = obj.optString("clothes_id"); //상품 번호만 추출
    System.out.println("-> clothes_id" +clothes_id);
    contentsno_list.add(clothes_id);
  }

  hashMap.put("contentsno_list",contentsno_list);
  
  // 추천 상품 목록 읽기
  
 ArrayList<ContentsVO> list = this.contentsProc.mf_clothes_member(hashMap);
 mav.addObject("list", list);
  
 //테이블 유형
///webapp/WEB-INF/views/contents/contents/mf_movie_member.jsp
 mav.setViewName("/contents/mf_clothes_member"); 
 
 //그리드 유형
 // /webapp/WEB-INF/views/contents/mf_movie_member_grid.jsp
 mav.setViewName("/contents/mf_clothes_member_grid"); 
